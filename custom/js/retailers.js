...
$(document).ready(function() {
	...
	manageBrandTable = $("#manageBrandTable").DataTable({
		'ajax': 'php_action/fetchRetailers.php',
		'order': []		
	});
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
        ...
        if(mobnumber == "") {
			$("#mobnumber").after('<p class="text-danger">Mobile No field is required</p>');
			$('#mobnumber').closest('.form-group').addClass('has-error');
		} else {
			// remov error text field
			$("#mobnumber").find('.text-danger').remove();
			// success out for form 
			$("#mobnumber").closest('.form-group').addClass('has-success');	  	
		}
		if(brandStatus == "") {
